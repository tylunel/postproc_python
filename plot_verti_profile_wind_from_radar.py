...
save_plot = True
save_folder = f'./figures/verti_profile_radar/{site}/'
...
if site == 'elsplans':
    source_obs_list = ['uhf', 'lidar', 'mast']
...
#    ds_temp['time_dist'] = np.abs(np.array([pd.Timestamp(time) - pd.Timestamp(wanted_date) for time in ds_temp.time.values]))
    ...
    source_obs_list = ['uhf', 'windcube', 'mast']
...
for source in source_obs_list:
    ...
    ax[0].plot(obs_dict[source]['WS'], obs_dict[source].level_agl, 
    ...
    ax[1].plot(obs_dict[source]['WD'], obs_dict[source].level_agl, 
               label=f'obs_{source}_{obstime}',
               color=colordict[f'obs_{source}'],
               marker=markerdict[f'obs_{source}'],
               linestyle=':',
