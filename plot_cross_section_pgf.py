...
varname_colormap = 'PGF'
# values color for contourf plot
vmin = 0  #0 for pgf
vmax = 0.00015  #0.00015 for pgf
colormap='jet'

varname_contourmap = None
...
# Datetime
wanted_date = '20210717-1500'
# altitude ASL or height AGL: 'asl' or 'agl'
alti_type = 'agl'
# maximum level (height AGL) to plot
toplevel = 1000
# array of altitudes (min of 3 levels for gradient computation)
alti_arr = np.arange(100,250,50)
...
figsize = (12,6)
save_plot = True
save_folder = './figures/cross_sections/{0}/section_{1}_{2}/{3}/pgf/'.format(
...
ds_agl_in = tools.subset_ds(data[var_list], zoom_on='urgell')
ds_asl = tools.agl_to_asl_coords(ds_agl_in, alti_arr)


#alti_arr = np.arange(25,1000,25)
#var_list = ['DENS', 'PRES', 'THT']
#
#ds_asl = data[['THT', 'ZS']].isel(level=np.arange(len(alti_arr)))
#
#for var in var_list:
#    ds_asl[f'{var}_ASL'] = ds_asl['THT']*0
#    ds_asl[f'{var}_ASL'] = ds_asl['THT']*0
#
## change levels from AGL to ASL
#ds_asl['level'] = alti_arr
##ds_asl['ZS'] = data['ZS']
#
## remove vars that are not ASL
##ds_asl = ds_asl.drop_vars(['THT','RVT'])  # Now added in var_list
#
## interpolation and load
#for ilevel, alti in enumerate(alti_arr):
#    print(f'Computation for {alti}m')
#    for var in var_list:
#        ds_asl[f'{var}_ASL'][ilevel,:,:] = tools.interp_iso_asl(alti, data, var, 
#                                                             verbose=False)

ds_asl['PRES_GRAD_W'], ds_asl['PRES_GRAD_U'], ds_asl['PRES_GRAD_V'] = \
    mcalc.gradient(ds_asl['PRES_ASL'].squeeze()[:, :, :])        
        
ds_asl['PGF_U'] = -(1/ds_asl['DENS_ASL'])*ds_asl['PRES_GRAD_U']
ds_asl['PGF_V'] = -(1/ds_asl['DENS_ASL'])*ds_asl['PRES_GRAD_V']
ds_asl['PGF_W'] = -(1/ds_asl['DENS_ASL'])*ds_asl['PRES_GRAD_W']

ds_asl['PGF'], ds_asl['PGF_dir'] = tools.calc_ws_wd(ds_asl['PGF_U'], ds_asl['PGF_V'])
...
data['PROJ'] = tools.windvec_verti_proj(data['PGF_U'], data['PGF_V'], 
                                       data.level, angle)
