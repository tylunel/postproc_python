...
save_plot = True
save_folder = f'./fig/verti_profiles/winds/'
...
for model in simu_list:     # model will be 'irr' or 'std'
    ...
    #         'irrlagrip30_d1': -pd.Timedelta(30, 'm'),
    #         'std_d1': -pd.Timedelta(60, 'm'),
    #         }
    # wanted_date_model = pd.Timestamp(wanted_date) + dict_marinada_arrival_timedelta[model]
    wanted_date_model = wanted_date
    print(wanted_date_model)
...
#        wd_1d_std = wd_3d_column.std(dim=['nj', 'ni'])
        ...
        ut_1d = var3d_low.isel(nj=index_lat, ni=index_lon)['UT']
        vt_1d = var3d_low.isel(nj=index_lat, ni=index_lon)['VT']
    ...
    # Wind Direction
    ax[1].plot(wd_1d, ws_1d.level,  # ws_1d used because wd_1d has no level
             ls='-', 
             color=colordict[model], 
             label=f'simu_{simu_names[model]}',
             )
    
    if mean_profile:  # add standard deviation
        # Wind Speed
        ax[0].fill_betweenx(
                ws_1d.level,
