...
figsize = (8,7)
save_plot = True
save_folder = f'./fig/linear_cross_sections/'
...
if add_wind_barbs:
    ...
    gridspec_kw = {'height_ratios': [4]*(nb_subplot-1) + [3]}
    wind_barbs_yticks = []
else:
...
for i_model, model in enumerate(models):
    ...
    for i, var_name_bu in enumerate(var_to_plot_bu):
        ...
        ax[i].grid()
        ax[i].legend(loc='lower left', fontsize=9)
    ...
    for i, var_name in enumerate(var_to_plot):
        ...
        if 'FROUDE' in var_name:
            ...
            ax[i_ax].set_ylim(0,6)
        elif 'HLOWJET' in var_name:
            ax[i_ax].set_ylim(0,750)
    ...
    if add_wind_barbs:
        ...
        ax[nb_subplot-1].barbs(
            ...
            section_ds['i_sect'][::skip_barbs]*0 + i_model, 
...
abscisse_sites_xaxis = {key:gv.whole[val]['acronym'] for (key,val) in zip(abscisse_sites.keys(), abscisse_sites.values())}
...
if add_wind_barbs:
    ...
    ax[-2].set_xticklabels(list(abscisse_sites_xaxis.values()), 
...
### Global options ---
plot_title = f'{wanted_date[-4:-2]}:{wanted_date[-2::]}'
fig.suptitle(plot_title)
save_title = f'{wanted_date}-{line_level}m agl-{var_to_plot}-{var_to_plot_bu}'
