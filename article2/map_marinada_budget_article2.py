...
save_folder = f'./fig/zonal_maps_budget/{ilevel_low}-{ilevel_high}/'
...
cbar = fig1.colorbar(pcm, ax=ax, boundaries=[vmin, vmax])
...
#            verti_compo=abs(float(layer_for_fig.mean()))
        ...
        if it_area == 0:
            ...
            lon_arrow = lon_bottom_left + it_var*0.04
            lon_legend = lon_bottom_left + it_var*0.04 - 0.028 
            ...
            if it_var == 0:  # for first value plotted, add a title and scale
                ...
                ax.arrow(lon_bottom_left - 0.04, lat_bottom_left, 
                         0, scale_val_arrow,
                         width=arrow_width, color='k',
                         )
                ax.text(lon_bottom_left - 0.07, lat_bottom_left, 
