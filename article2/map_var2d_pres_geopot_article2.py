...
wanted_date = '20210721-1400'

color_map = 'coolwarm_r'   # coolwarm, plasma, YlGnBu_r, ... (add _r to reverse)
                    # YlOrBr for orography
                    # RdYlGn for LAI

var_name = 'MSLP'   #LAI_ISBA, ZO_ISBA, PATCHP7, ALBNIR_S, MSLP, TG1_ISBA, RAINF_ISBA, CLDFR, TSWI_T_ISBA
vmin, vmax = 990, 1030
...
#%% add geopotential
pres = ds['PRES'].squeeze()
pressure_of_geopot = 500

level_index = np.abs((pres - pressure_of_geopot)).argmin(dim='level')
# Extract data at the target level

data_at_500hPa = pres.isel(level=level_index)

isobaric_level_agl = level_index*0
for j, valj in enumerate(pres.nj):
    print(j)
    for i, vali in enumerate(pres.ni):
        isobaric_level_agl[j, i] = ds.level[level_index[j, i]]

isobaric_level_asl = isobaric_level_agl + ds['ZS']

geopot_height = isobaric_level_asl/10


out_h5800 = tools.interp_iso_asl(5800, ds, 'PRES')
out_h5900 = tools.interp_iso_asl(5900, ds, 'PRES')
out_h6000 = tools.interp_iso_asl(6000, ds, 'PRES')

if add_geopot:
    # plt.contour(geopot_height.longitude.data, 
    #             geopot_height.latitude.data, 
    #             geopot_height,
    #             levels=[500],   #+1 -> number of contour to plot 
    #             linestyles='--',
    #             linewidths=1.,
    #             colors='k',
    #             )
    plt.contour(
        geopot_height.longitude.data, 
        geopot_height.latitude.data, 
        out_h6000,
        levels=[500],
        linestyles='--',
