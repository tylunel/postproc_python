...
color_map = 'YlOrBr'   # coolwarm, plasma, YlGnBu_r, ... (add _r to reverse)
                    # YlOrBr for orography
                    # RdYlGn for LAI

var_name = 'ZS'   #LAI_ISBA, ZO_ISBA, PATCHP7, ALBNIR_S, MSLP, TG1_ISBA, RAINF_ISBA, CLDFR, TSWI_T_ISBA
vmin = 0.1
vmax = 1500
        ...
        if os.path.isfile(file_path):
            try:
                ...
                if (lon_range[0] < obs['lon'] < lon_range[1] and \
                    ...
                    #              wind_height, 
                    #              fontsize=7)
                    # add station name next to the point
                    ax.text(obs['lon']+0.008, obs['lat']-0.018, 
                            station_name, 
                            fontsize=11)
                    # state that this station was plotted
                    print(f'{station_name} plotted')
                    # put the station name in a dict
                    smc_dict_kept[station_name] = {
...
topo_list = ['serra_tallat',
             'serra_miramar',
             'torredembarra',]

sites = {key:gv.whole[key] for key in sites_list}
topo = {key:gv.whole[key] for key in topo_list}
...
for site in topo:
    ...
    # print site name on fig:
    try:
        sitename = topo[site]['acronym']
    except KeyError:
        sitename = site
        
    plt.text(topo[site]['lon']+0.01,
             topo[site]['lat']+0.01, 
             sitename, 
#             fontsize=12
             )
...
plot_title = 'smc_stations_locations'
