...
var_to_plot = ['HLOWJET_07', 'FROUDE_HJET_07',]
#vmin, vmax = 0, 1000
hlowjet_max = 750
...
figsize = (12,7)
save_plot = False
save_folder = './fig/linear_cross_sections/'

plt.rcParams.update({'font.size': 11})

###########################################
simu_names = {'irrswi1_d1': 'FC_IRR',
...
longnames_dict = {
            'THTV': 'virtual potential\n temperature [K]',
            'THT': 'potential\n temperature [K]',
            'PRES_PROJ': 'PGF [m s$^{-1}$]',
            'FROUDE_HJET_07': 'Froude number',
            'HLOWJET_07': 'Jet height - h$_{top}$ [m]',
            }

cmap = matplotlib.cm.get_cmap('Blues')

colordict = {
    '20210716-1700': cmap(0.5),
    '20210716-1800': cmap(0.5),
    '20210716-2100': cmap(0.6),
    '20210716-2200': cmap(0.7),
    '20210716-2300': cmap(0.8),
    '20210717-0000': cmap(0.9),
    '20210717-0100': cmap(0.99),
    '20210721-1200': 'b',
    '20210721-1400': 'g',
    '20210721-1600': 'r',
    '20210721-1800': 'b',
...
linedict = {
    ...
    '20210716-2100': (0, (5, 5)),
    '20210716-2200': (0, (7, 3)),
    '20210716-2300': (0, (5, 2, 1, 2)),
    '20210717-0000': (0, (4, 2, 2, 2)),
    '20210717-0100': (0, (3, 2, 3, 2)),
    '20210721-1200': (0, (5, 2, 1, 2)),
    '20210721-1400': (0, (2, 2, 1, 2)),
    '20210721-1600': (0, (5, 2)),
    '20210721-1800': (0, (5, 2, 1, 2)),
    '20210721-2000': (0, (2, 2, 1, 2)),
    '20210721-2200': (0, (5, 2)),
    }
...
for wanted_date in wanted_date_list:
# for model in models:
    ...
    data_reduced = ds[['UT', 'VT', 'WT', 'WS', 'THT', 'THTV', 'ZS',
#                       *var_to_plot
                       ]]
...
#%% STANDARD DATA
    ...
    if var_to_plot != []:
        ...
        for i, ni in enumerate(ni_range):
            ...
            if type(level_range) in [list, np.ndarray] and not var_in_diag_list:
        ...
        if var_in_diag_list:
            ...
            # filter HJET values
            if wanted_date in ['20210717-0100', '20210716-1700']:
                section_ds['HLOWJET_07'] = section_ds['HLOWJET_07'].where(
                    section_ds['HLOWJET_07'] < 700)
...
#            section_ds = section_ds.isel(level=line_level)
            section_ds.sel(level=line_level, method='nearest')
       
        section_ds_dict[wanted_date] = section_ds
...
#%% BUDGET DATA
    ...
    if var_to_plot_bu != []:
        ...
        section_ds_bu_dict[wanted_date] = section_ds_bu
...
for i_date, wanted_date in enumerate(wanted_date_list):
    
    # budget subplots
    for i, var_name_bu in enumerate(var_to_plot_bu):
        # loading here only allows not to load if empty dict
        section_ds_bu = section_ds_bu_dict[wanted_date]
        
        var_plot = f'{var_name_bu}_PROJ'
        ax[i].plot(section_ds_bu['i_sect'], section_ds_bu[var_plot],
                   label=wanted_date,
                   # color='k',
                   color=colordict[wanted_date],
                    linestyle=linedict[wanted_date],
                   )
        ax[i].set_ylabel(longnames_dict[var_plot])
...
#        ax[i].legend()
    ...
    for i, var_name in enumerate(var_to_plot):
        # loading here only allows not to load if empty dict
        section_ds = section_ds_dict[wanted_date]
        ...
        ax[i_ax].set_ylabel(longnames_dict[var_name])
        ax[i_ax].set_xticklabels([])  # remove x axis ticks labels
        ax[i_ax].set_xlim(section_ds['i_sect'].min(), section_ds['i_sect'].max())
        # ax[i_ax].legend()
        if 'FROUDE' in var_name:
            #add horizontal line for value 1
            ax[i_ax].hlines(1, 
                xmin=section_ds['i_sect'].data.min(),
                xmax=section_ds['i_sect'].data.max(),
                color='k', linestyle='-',
                )
            ax[i_ax].set_ylim(0, 15)
            ax[i_ax].set_yscale('log')
        elif 'HLOWJET' in var_name:
            ax[i_ax].set_ylim(0,hlowjet_max)            
    ...
    #         section_ds['i_sect'][::skip_barbs], 
    #         section_ds['i_sect'][::skip_barbs]*0 + i_date, 
    #         #Here dimensions are in the proper order
    #         section_ds['UT'][::skip_barbs], 
    #         section_ds['VT'][::skip_barbs], 
    #         pivot='middle',
    ...
    #     wind_barbs_yticks.append(simu_names[model])
...
for i in range(nb_subplot):
    ax[i].set_xticks(list(abscisse_sites_xaxis.keys()))
...
#        'right', 'left'
    ...
    ax[-1].set_xticklabels(list(abscisse_sites_xaxis.values()), 
                   rotation=0, fontsize=12)
...
### add secondary axis ---
# create place for secondary axis
...
### Global options ---
plot_title = f'multitime-{model}-{line_level}m-{var_to_plot}-{var_to_plot_bu}'
