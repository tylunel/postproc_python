...
varname_sim_list = ['LE_ISBA', 
                    'H_ISBA', 'RN_ISBA', 'U_STAR'
                    ]
# varname_sim = 'LWU_ISBA'    # TG3_ISBA, LE_ISBA, LWU_ISBA, TG2_ISBA, x, U_STAR, SWI4_ISBA
# varname_obs = 'LW_OUT'  # T_10cm_Avg, LE, LW_OUT

varname_obs_dict = {'LE_ISBA': 'LE', 'H_ISBA': 'H', 
                    'RN_ISBA': 'NETRAD', 'U_STAR': 'U_STAR'}
...
# -- For IRTA-corn
#LE, H, FC_mass, WS, WD, U_STAR, SW_OUT, NETRAD
...
one_day_zoom_plot = 10  # just zoom on one day (do not necessarily remove the rest)
...
for varname_sim in varname_sim_list:
    ...
    rmse_var[varname_sim] = rmse
    bias_var[varname_sim] = bias
...
#%%
table_bias = pd.DataFrame(bias_var)
table_rmse = pd.DataFrame(rmse_var)
print(' --- LATEX TABLE BIAS ---- ')
# print(table_bias.to_latex())
print(table_bias.to_latex(index=False, float_format="%.3g"))
print(' --- LATEX TABLE RMSE ---- ')
print(table_rmse.to_latex(index=False, float_format="%.3g"))
