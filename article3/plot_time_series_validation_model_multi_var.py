...
varname_obs_dict = {'LE': 'LE', 'H': 'H', 'U_STAR': 'U_STAR', 'RN': 'NETRAD',
                    'LWU': 'LW_OUT', 'SWU': 'SW_OUT', 'TG3_ISBA': 'T_10cm_Avg',
                    'TALB_ISBA': 'ALB', 'ALB_MANUAL': 'ALB',
                    }
varname_obs = 'LE'
# -- For CNRM:
...
for varname_sim in varname_sim_list:
    varname_obs = varname_obs_dict[varname_sim]
    ...
    if varname_sim in ['TG2_ISBA', 'TG3_ISBA']:
        offset_obs = 273.15
        ylabel = varname_sim
        secondary_axis = False
    if varname_sim in ['TALB_ISBA', 'ALB_MANUAL']:
        coeff_obs = 0.01
        ylabel = varname_sim
        secondary_axis = False
    else:
    ...
    if varname_obs != '':
        ...
        if one_day_focus not in [0, None]:
            # obs = obs.where(xr.DataArray(pd.DatetimeIndex(obs.time).day) == one_day_focus,
            #                 drop=True)
            obs = obs.where(obs.time<pd.Timestamp('20210731T0000'), drop=True)
            obs = xr.where(pd.DatetimeIndex(obs.time).day == one_day_focus, 
                           obs, np.nan).dropna(dim='time', how='all')
...
#                label='obs_'+varname_obs,
        ...
        if add_seb_residue:
            ...
            if varname_obs in ['LE', 'lhf_1']:
                ...
                print(f'seb_residue not applied on {varname_sim}')
                plot_residue_line = False
                obs_residue_corr = obs_var_corr
                obs_uncertainty = obs['SEB_RESIDUE'].data * 0
            
            if plot_residue_line:
...
#%% SIMU:
    ...
    datafolder = (
        ...
        'et_overestimation/calibration/{0}/forcing_{1}_{2}/{3}/'.format(
            site, forcing_file, cphoto, model))
    #    datafolder = folder + 'forcing_{0}/'.format(models_idnumber[model])
    
    if varname_sim in ['TG2_ISBA', 'TG3_ISBA', 'TALB_ISBA',]:
    ...
    if varname_sim == 'U_STAR':
        ds['U_STAR'] = tools.calc_u_star_sim(ds.FMU, ds.FMV)
    elif varname_sim == 'ALB_MANUAL':
        ds['ALB_MANUAL'] = ds['SWU']/ds['SWD']
...
#             label='simu_{0}_{1}'.format(model, varname_sim),
    ...
    if errors_computation:
        ## Errors computation
        obs_sorted[varname_sim] = []
        sim_sorted[varname_sim] = []
...
#        time_window = ds.time[:][(day-1)*int(24/timestep) : (day)*int(24/timestep)]
        ...
        for i, date in enumerate(time_window):
            ...
            if len(val) == 0:
                pass
            if len(val) == 1:
                sim_sorted[varname_sim].append(var_1d[i])
                obs_sorted[varname_sim].append(float(val))
            elif len(val) == 2:
                sim_sorted[varname_sim].append(var_1d[i])
                obs_sorted[varname_sim].append(float(val[0]))
                print('Weird: val = {0} at date {1}'.format(val, date))
             
        diff[varname_sim] = np.array(sim_sorted[varname_sim]) - np.array(obs_sorted[varname_sim])
        # compute bias and rmse, and keep values with 3 significant figures
        bias[varname_sim] = float('%.3g' % np.nanmean(diff[varname_sim]))
#        rmse[model] = np.sqrt(np.nanmean((np.array(obs_sorted[model]) - np.array(sim_sorted[model]))**2))
        rmse[varname_sim] = float('%.3g' % np.sqrt(np.nanmean(diff[varname_sim]**2)))
...
#%% Plot esthetics
    ...
    if one_day_focus not in [0, None]:
        xmin = pd.Timestamp(f'202107{one_day_focus}T0000')
        xmax = pd.Timestamp(f'202107{one_day_focus}T2330')
    ...
    save_title = f'{ylabel}_at_{site}_validation_{varname_sim}'
