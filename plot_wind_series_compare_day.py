...
ilevel = 3   #0 is Halo, 1->2m, 2->6.12m, 3->10.49m, 10 -> 50m
...
def plot_wind_speed(ws, dates=None, start_date=None, end_date=None, 
    ...
    ax1.plot(dates, ws, 
             ...
             **kwargs)
    ax1.set_ylabel('Wind Speed [m s$^{-1}$]', multialignment='center')
...
for model in simu_folders:
    ...
    if out_suffix == '.OUT':
        freq_simu = 30
        dati_arr_sim = np.array([start + np.timedelta64(i*freq_simu, 'm') for i in np.arange(0, ds1['record'].shape[0])])
    else:
        freq_simu = 60
        dati_arr_sim = np.array([start + np.timedelta64(i, 'h') for i in np.arange(0, ut_md.shape[0])])
        
    dati_arr_simu_alt = pd.date_range(start=pd.Timestamp(start), 
                                 periods=len(ds1['record']),
                                 freq=f'{freq_simu}T')
...
if plot_multi_days:
    ...
    for i, day in enumerate(days_list):
        ...
        # OBS
        day_data_ws = ws_obs_filtered[dati_arr_obs.day == day]
        day_data_wd = wd_obs[dati_arr_obs.day == day]
        ...
        ax[0].plot(time_dati_arr_obs, day_data_ws, 
             label=f'obs_{day}',
             color=colordict['obs'],
             linewidth=1.5,
             linestyle=linestyle_list[i],
             )
        ax[1].plot(time_dati_arr_obs, day_data_wd, 
             label=f'obs_{day}',
             color=colordict['obs'],
             linewidth=1.5,
             linestyle=linestyle_list[i],
        ...
        day_simu_ws = ws[dati_arr_simu_alt.day == day]
        day_simu_wd = wd[dati_arr_simu_alt.day == day]
        
        minute_dati_arr_simu = dati_arr_simu_alt[dati_arr_simu_alt.day == day].minute / 60
        hour_dati_arr_simu = dati_arr_simu_alt[dati_arr_simu_alt.day == day].hour
        time_dati_arr_simu = minute_dati_arr_simu + hour_dati_arr_simu
    
        ax[0].plot(time_dati_arr_simu, day_simu_ws, 
             label=f'simu_{day}',
             color=colordict[model],
             linewidth=1.5,
             linestyle=linestyle_list[i],
             )
        ax[1].plot(time_dati_arr_simu, day_simu_wd, 
             label=f'simu_{day}',
             color=colordict[model],
             linewidth=1.5,
             linestyle=linestyle_list[i],
    ...
    ax[0].set_ylabel('Wind Speed [m s$^{-1}$]', multialignment='center')
    ax[0].grid(visible=True, which='major', axis='y', color='k', linestyle='--',
             linewidth=0.5)
        
    ax[1].set_ylabel('Wind Direction [°]', multialignment='center')
    ax[1].set_ylim(0, 360)
    ax[1].set_yticks(np.arange(0, 360, 90))
    ax[1].set_yticklabels(['0 (N)', '90 (E)', '180 (S)', '270 (W)'])
    ...
    # add grey zones for night
    ax[0].axvspan(19.5, 24, ymin=0, ymax=1, 
                   color = '0.9'  #'1'=white, '0'=black, '0.8'=light gray
                   )
    ax[1].axvspan(19.5, 24, ymin=0, ymax=1, 
                   color = '0.9'  #'1'=white, '0'=black, '0.8'=light gray
                   )
