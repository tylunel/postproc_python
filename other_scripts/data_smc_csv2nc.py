...
#%% Other option to get a LaTeX table from a dataframe
file = '/home/lunelt/Data/data_LIAISE/SMC/stations_list.csv'

df1 = pd.read_csv(file, 
                  names=['region', 'name', 'name_code', 'code',
                         'lat', 'lon', 'alt', 'creation_date'])
df_temp = df1
df_temp = df_temp[df_temp['lat'] > gv.zoom_domain_prop['marinada']['lat_range'][0]]
df_temp = df_temp[df_temp['lat'] < gv.zoom_domain_prop['marinada']['lat_range'][1]]
df_temp = df_temp[df_temp['lon'] > gv.zoom_domain_prop['marinada']['lon_range'][0]]
df_temp = df_temp[df_temp['lon'] < gv.zoom_domain_prop['marinada']['lon_range'][1]]

df_temp = df_temp.set_index('code')
df_temp = df_temp.drop(['region', 'name_code', 'creation_date'], axis=1)
df_temp.to_latex()
