...
for model in models:
    ...
    for i, var_name in enumerate(var_to_plot):
        ...
        ax[i_ax].plot(section_ds['i_sect'], section_ds[var_plot],
                   label=model,
                   color=colordict[model])
        ax[i_ax].set_ylabel(var_plot)
        ax[i_ax].set_xticklabels([])  # remove x axis ticks labels
        ax[i_ax].set_xlim(section_ds_bu['i_sect'].min(), section_ds_bu['i_sect'].max())
        ax[i_ax].grid()
        ax[i_ax].legend()
